...
lsp_installer.on_server_ready(function(server)
  ...
  if server.name == 'tsserver' then
    opts = u.merge(opts, require('lsp.providers.tsserver'))
  elseif server.name == 'jsonls' then
    opts = u.merge(opts, require('lsp.providers.jsonls'))
  elseif server.name == 'pyright' then
    opts = u.merge(opts, require('lsp.providers.pyright'))
  elseif server.name == 'sumneko_lua' then
    opts = u.merge(opts, require('lsp.providers.sumneko_lua'))
  elseif server.name == 'volar' then
    opts = u.merge(opts, require('lsp.providers.volar'))
  end
